...
function currentRDPSurfaceSize(surface) { const bounds=surface.getBoundingClientRect(), width=Math.round(bounds.width), height=Math.round(bounds.height); return width>0&&height>0 ? {width,height} : null; }
async function initialRDPSurfaceSize(surface) { const measured=await RunPilotRDPSize.measureAfterLayout(nextAnimationFrame,()=>currentRDPSurfaceSize(surface)); const resolved=RunPilotRDPSize.resolveInitialSize(measured); if(resolved.fallback) console.warn("initial RDP surface unavailable; using fallback 640x480"); else console.debug(`initial RDP surface size: ${resolved.width}x${resolved.height}`); return resolved; }
...
function inspectGuacamoleDisplay(surface,display,reason) {
  requestAnimationFrame(()=>{
    const element=display.getElement(), bounds=element.getBoundingClientRect(), canvases=[...element.querySelectorAll("canvas")], canvasDetails=canvases.map(canvas=>{ const style=getComputedStyle(canvas), rect=canvas.getBoundingClientRect(); return {width:canvas.width,height:canvas.height,cssWidth:style.width,cssHeight:style.height,offsetWidth:canvas.offsetWidth,offsetHeight:canvas.offsetHeight,visibility:style.visibility,opacity:style.opacity,display:style.display,zIndex:style.zIndex,position:style.position,bounds:{x:rect.x,y:rect.y,width:rect.width,height:rect.height}}; });
    console.debug("Guacamole display diagnostics",{reason,container:{clientWidth:surface.clientWidth,clientHeight:surface.clientHeight},display:{offsetWidth:element.offsetWidth,offsetHeight:element.offsetHeight,width:display.getWidth(),height:display.getHeight(),scale:typeof display.getScale==="function" ? display.getScale() : null,bounds:{x:bounds.x,y:bounds.y,width:bounds.width,height:bounds.height}},canvasCount:canvasDetails.length,canvases:canvasDetails});
    const framebuffer=canvases.filter(canvas=>canvas.width>0&&canvas.height>0).sort((a,b)=>(b.width*b.height)-(a.width*a.height))[0];
    if (!framebuffer) { console.debug("Guacamole framebuffer canvas: none"); return; }
    try { const pixels=framebuffer.getContext("2d",{willReadFrequently:true}).getImageData(0,0,Math.min(4,framebuffer.width),Math.min(4,framebuffer.height)).data; let nonEmpty=false; for(let index=0;index<pixels.length;index+=4) { if(pixels[index]||pixels[index+1]||pixels[index+2]||pixels[index+3]) { nonEmpty=true; break; } } console.debug(`Guacamole framebuffer canvas: ${framebuffer.width}x${framebuffer.height}; sampled pixels: ${nonEmpty ? "non-empty" : "all transparent/blank"}`); } catch (error) { console.warn("Guacamole framebuffer pixel sample failed",{width:framebuffer.width,height:framebuffer.height,error:String(error?.message||error)}); }
  });
}
function instrumentGuacamoleDisplay(tunnel,display,surface) {
  const previousInstruction=tunnel.oninstruction, previousResize=display.onresize, imageDecoderAvailable=Boolean(window.ImageDecoder&&window.ReadableStream); let firstImageStream="", firstImageTracked=false, awaitingFirstImageSync=false, firstImageSynced=false, inspected=false, imageDecodePending=false, firstImageRenderPending=false, imageDecodeTimer=0;
  console.debug(`Guacamole display image decoder: ${imageDecoderAvailable ? "ImageDecoder" : "data URI Image fallback"}`);
  const imageDecodeRejection=event=>{ if(imageDecodePending) console.warn("Guacamole image decode promise rejected",{name:String(event.reason?.name||"unknown")}); }; window.addEventListener("unhandledrejection",imageDecodeRejection);
  const imageCSPViolation=event=>{ const blocked=String(event.blockedURI||""); if(firstImageRenderPending&&(blocked.startsWith("data:")||blocked.startsWith("blob:"))) console.warn("Guacamole image CSP violation",{directive:String(event.effectiveDirective||"unknown"),source:blocked.split(":",1)[0]+":"}); }; document.addEventListener("securitypolicyviolation",imageCSPViolation);
  tunnel.oninstruction=(opcode,args)=>{ const value=index=>String(args?.[index]??""); if(opcode==="img"&&!firstImageTracked&&args?.length>=4) { firstImageStream=value(1); firstImageTracked=true; firstImageRenderPending=true; imageDecodePending=imageDecoderAvailable; console.debug(`browser received img stream=${firstImageStream} layer=${value(2)} mimetype=${value(3)}`); } else if(opcode==="blob"&&firstImageStream!==""&&value(0)===firstImageStream) console.debug(`browser received blob stream=${firstImageStream} length=${value(1).length}`); else if(opcode==="end"&&firstImageStream!==""&&value(0)===firstImageStream) { console.debug(`browser received end stream=${firstImageStream}`); firstImageStream=""; awaitingFirstImageSync=true; } else if(opcode==="sync"&&awaitingFirstImageSync) { console.debug(`browser received sync timestamp=${value(0)}`); awaitingFirstImageSync=false; firstImageSynced=true; clearTimeout(imageDecodeTimer); imageDecodeTimer=setTimeout(()=>{ imageDecodePending=false; firstImageRenderPending=false; },10000); }
    previousInstruction?.(opcode,args);
    if(opcode==="sync"&&firstImageSynced&&!inspected) { inspected=true; inspectGuacamoleDisplay(surface,display,"first image sync"); }
  };
  display.onresize=(width,height)=>{ console.debug(`Guacamole display resized to ${width}x${height}`); previousResize?.(width,height); };
  return ()=>{ clearTimeout(imageDecodeTimer); window.removeEventListener("unhandledrejection",imageDecodeRejection); document.removeEventListener("securitypolicyviolation",imageCSPViolation); tunnel.oninstruction=previousInstruction; display.onresize=previousResize; };
}
async function openRDPRemoteSession(session, credentials) {
  const ticket=await api(`api/v1/remote/sessions/${session.id}/transport-ticket`,{method:"POST"}), surface=$("remoteRDP");
  $("remoteFrame").src="about:blank"; $("remoteFrame").classList.add("hidden"); surface.classList.remove("hidden"); surface.replaceChildren();
  const initialSize=await initialRDPSurfaceSize(surface), transportParams=remoteTransportParams(ticket.ticket,{width:initialSize.width,height:initialSize.height,dpi:window.devicePixelRatio > 1 ? 120 : 96,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||""});
  const tunnel=new Guacamole.WebSocketTunnel(remoteTransportURL(session.id)); const client=new Guacamole.Client(tunnel), display=client.getDisplay(), stopDisplayInstrumentation=instrumentGuacamoleDisplay(tunnel,display,surface); surface.append(display.getElement());
  const sizing=RunPilotRDPSize.createController({resizeMethod:session.rdp?.resizeMethod||"display-update",initialSize,getSurfaceSize:()=>currentRDPSurfaceSize(surface),sendRemoteSize:size=>client.sendSize(size.width,size.height),fitLocal:(available,remote)=>{ const scale=RunPilotRDPSize.fitScale(available,remote); if(scale!==null) display.scale(scale); }}), previousDisplayResize=display.onresize;
  ...
  remoteRDPInteraction={shutdown:()=>{ sizing.dispose(); stopDisplayInstrumentation(); keyboard.reset(); keyboard.onkeydown=null; keyboard.onkeyup=null; mouse.onEach(["mousedown","mousemove","mouseup"],null); client.disconnect(); }, focus:()=>surface.focus({preventScroll:true}), surfaceChanged:()=>sizing.surfaceChanged()};
  surface.addEventListener("pointerdown",()=>surface.focus({preventScroll:true})); const observer=new ResizeObserver(()=>sizing.surfaceChanged()); observer.observe(surface); const prior=remoteRDPInteraction.shutdown; remoteRDPInteraction.shutdown=()=>{observer.disconnect();prior()};
