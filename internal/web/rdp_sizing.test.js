...
test("reconnect mode sends the final settled size to let guacd reconnect unsupported targets",()=>{
  let surface={width:1220,height:1038}; const sent=[];
  const controller=sizing.createController({resizeMethod:"reconnect",initialSize:surface,getSurfaceSize:()=>surface,sendRemoteSize:size=>sent.push(size)});
  controller.setConnected(true);
  surface={width:1512,height:1204}; controller.surfaceChanged(); controller.flushPendingResize();
  assert.deepEqual(sent,[{width:1512,height:1204}]);
});
...
test("fullscreen and exit use the same settled display-update controller",()=>{
  let surface={width:1220,height:1038}; const sent=[];
  const controller=sizing.createController({resizeMethod:"display-update",initialSize:surface,getSurfaceSize:()=>surface,sendRemoteSize:size=>sent.push(size)});
  controller.setConnected(true);
  surface={width:2560,height:1440}; controller.surfaceChanged(); controller.flushPendingResize();
  surface={width:1220,height:1038}; controller.surfaceChanged(); controller.flushPendingResize();
  assert.deepEqual(sent,[{width:2560,height:1440},{width:1220,height:1038}]);
});

test("a sidebar transition fits intermediate sizes but requests only its final remote size",()=>{
  let surface={width:1220,height:1038}; const sent=[];
  const controller=sizing.createController({resizeMethod:"display-update",initialSize:surface,getSurfaceSize:()=>surface,sendRemoteSize:size=>sent.push(size)});
  controller.setConnected(true); controller.layoutTransitionStarted();
  surface={width:1248,height:1038}; controller.surfaceChanged();
  surface={width:1282,height:1038}; controller.surfaceChanged();
  surface={width:1340,height:1038}; controller.surfaceChanged();
  controller.flushPendingResize();
  assert.deepEqual(sent,[]);
  controller.layoutTransitionFinished(); controller.flushPendingResize();
  assert.deepEqual(sent,[{width:1340,height:1038}]);
});

test("a display resize alone recalculates local fit without a resize request loop",()=>{
  const surface={width:1220,height:1038}, sent=[], fits=[];
  const controller=sizing.createController({resizeMethod:"display-update",initialSize:surface,getSurfaceSize:()=>surface,sendRemoteSize:size=>sent.push(size),fitLocal:(available,remote)=>fits.push({available,remote})});
  controller.setConnected(true); controller.displayResized({width:1220,height:1038}); controller.flushPendingResize();
  controller.displayResized({width:1200,height:900}); controller.flushPendingResize();
  assert.deepEqual(sent,[]);
